...
internal interface BigTextAdapter {
    ...
    fun binding(binding: ItemTextBinding, viewType: Int, position: Int, item: BigTextViewItem, payloads: MutableList<Any>) {
    ...
    fun binding(binding: ItemTextBinding, viewType: Int, position: Int, item: BigTextViewItem) {
    ...
    private fun refreshSize(binding: ItemTextBinding, item: BigTextViewItem) {

        binding.root.setSize(item.size)
    }

    private fun refreshMargin(binding: ItemTextBinding, item: BigTextViewItem) {

        binding.root.setMargin(item.margin)
    }

    private fun refreshPadding(binding: ItemTextBinding, item: BigTextViewItem) {

        binding.root.setPadding(item.padding)
    }

    private fun refreshBackground(binding: ItemTextBinding, item: BigTextViewItem) {

        binding.root.setBackground(item.background)
    }


    private fun refreshText(binding: ItemTextBinding, item: BigTextViewItem) {

        binding.tvTitle.setText(item.text)
    }

    private fun refreshTextStyle(binding: ItemTextBinding, item: BigTextViewItem) {

        binding.tvTitle.setTextStyle(item.textStyle)
    }

    private fun refreshTextSize(binding: ItemTextBinding, item: BigTextViewItem) {

        binding.tvTitle.setSize(item.textSize)
    }

    private fun refreshTextMargin(binding: ItemTextBinding, item: BigTextViewItem) {

        binding.tvTitle.setMargin(item.textMargin)
    }

    private fun refreshTextPadding(binding: ItemTextBinding, item: BigTextViewItem) {

        binding.tvTitle.setPadding(item.textPadding)
    }

    private fun refreshTextBackground(binding: ItemTextBinding, item: BigTextViewItem) {
    ...
    private fun refreshImageLeft(binding: ItemTextBinding, item: BigTextViewItem) {
        ...
        // Note: setImage might need adaptation if it expects specific types
        binding.ivLeft.setImage(item.imageLeft ?: 0)
    }

    private fun refreshImageLeftSize(binding: ItemTextBinding, item: BigTextViewItem) {

        binding.ivLeft.setSize(item.imageLeftSize)
    }

    private fun refreshImageLeftMargin(binding: ItemTextBinding, item: BigTextViewItem) {

        binding.ivLeft.setMargin(item.imageLeftMargin)
    }

    private fun refreshImageLeftPadding(binding: ItemTextBinding, item: BigTextViewItem) {

        binding.ivLeft.setPadding(item.imageLeftPadding)
    }

    private fun refreshImageLeftBackground(binding: ItemTextBinding, item: BigTextViewItem) {

        binding.ivLeft.setBackground(item.imageLeftBackground)
    }


    private fun refreshImageRight(binding: ItemTextBinding, item: BigTextViewItem) {

        binding.ivRight.setImage(item.imageRight ?: 0)
    }

    private fun refreshImageRightSize(binding: ItemTextBinding, item: BigTextViewItem) {

        binding.ivRight.setSize(item.imageRightSize)
    }

    private fun refreshImageRightMargin(binding: ItemTextBinding, item: BigTextViewItem) {

        binding.ivRight.setMargin(item.imageRightMargin)
    }

    private fun refreshImageRightPadding(binding: ItemTextBinding, item: BigTextViewItem) {

        binding.ivRight.setPadding(item.imageRightPadding)
    }

    private fun refreshImageRightBackground(binding: ItemTextBinding, item: BigTextViewItem) {

        binding.ivRight.setBackground(item.imageRightBackground)
    }
}

open class BigTextViewItem : ViewItem {
    ...
    open var text: BigText = emptyText()
